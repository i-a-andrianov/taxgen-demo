...
train_idx_features_labels = np.genfromtxt(
    "src/node_graph_reconstruct_model_directed.txt", dtype=np.dtype(str)
)
print(train_idx_features_labels.shape)
gbert_embs = {}
for i in range(train_idx_features_labels.shape[0]):
    gbert_embs[train_idx_features_labels[i, 0]] = np.array(
        train_idx_features_labels[i, 1:], np.dtype(float)
    )
...
class ProjectionLayer(nn.Module):
    def __init__(
        self, emb_size=300, hidden_sizes=[1024, 512], target_size=300, device="cuda"
    ):
        super().__init__()
...
model = ProjectionLayer(
    emb_size=300, hidden_sizes=HIDDEN_STATES, target_size=768, device=DEVICE
).to(DEVICE)
model.load_state_dict(
    torch.load("src/projection_model_gb-b.pt", map_location=torch.device("cpu"))
)
model.eval()
...
def filter_results(res):
    ...
    for r in res:
        if isinstance(r[0], list):
            ...
            if len(r_un_clean) > 0:
                if len(r_un_clean) == 1 and (
                    r_un_clean[0] not in [el[0] for el in res_filter]
                ):
                    res_filter.append((r_un_clean[0], r[1]))

                elif len(r_un_clean) > 1 and (
                    r_un_clean not in [el[0] for el in res_filter]
                ):
            ...
            if (
                is_alpha(r[0].lower())
                and not (r[0].lower() in stop_words)
                and (r[0].lower() not in [t[0] for t in res_filter])
            ):
...
def get_pred(inputs, subst_type="no", pred_child_emb=None, num=50):
    ...
    if subst_type == "sub":
        ...
        vec = torch.mean(
            torch.stack(
                [
                    hidden_states[:, masked_position, :].squeeze(),
                    torch.tensor(pred_child_emb).to(DEVICE),
                ]
            ),
            dim=0,
        )
    ...
    candidates = [
        i
        for i in candidates
        if all(not j.isdigit() and not j in string.punctuation for j in i[0])
    ]
...
def generate_candidates(word):
    pred_child_emb = model(
        torch.tensor(gbert_embs[word], dtype=torch.float32).to(DEVICE)
    )
    vowels = "aeiouy"
    ...
    candidates = filter_results(
        [
            i
            for i in mix
            if i[0] not in set(wn.all_lemma_names())
            and all(not j.isdigit() and not j in string.punctuation for j in i[0])
        ]
    )
